...
/**
 * A command that turns the robot in place to a given field heading.
 * <p>
 * Pedro Pathing 3.0 no longer has {@code Follower#turnTo}, so this holds the follower's current position with the new
...
public class TurnToCommand extends CommandBase {
     ...
     */
    public TurnToCommand setHeadingTolerance(double headingTolerance) {
     ...
     * @return This command for compatibility in command groups
     */
    public TurnToCommand setHeadingTolerance(double headingTolerance, AngleUnit angleUnit) {
    ...
    public void initialize() {
        follower.hold(follower.pose().withHeading(angle));
        follower.algorithm().reset();
    }

    @Override
    public boolean isFinished() {
        return Math.abs(Angle.error(follower.pose().heading(), angle)) < headingTolerance;
    }
}
